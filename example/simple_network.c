...
#define TRAINING_ITERATIONS 150001
...
static void create_network(void)
{
    ...
    result = uneural_network_set_layer_type(&output_layer, ACTIVATION_TYPE);
    if (result) {
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < TRAINING_ITERATIONS; i++) {
        ...
        if ((i % 10000) == 0) {
            ...
            global_error = fix16_sqrt(fix16_div(global_error, F16(4)));
            printf("RMSE: %f\n", fix16_to_float(global_error));
